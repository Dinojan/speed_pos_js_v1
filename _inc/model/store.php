...
class ModelStore extends Model 
{
	...
	{
    	$statement = $this->db->prepare("UPDATE `stores` SET `name` = ?, `code_name` = ?, `mobile` = ?, `email` = ?, `country` = ?,   `zip_code` = ?,  `address` = ?,   `deposit_account_id` = ? WHERE `store_id` = ? ");
    	$statement->execute(array($data['name'], $data['code_name'], $data['mobile'], $data['email'], $data['country'], $data['zip_code'], $data['address'],   $data['deposit_account_id'], $store_id));
	...
	public function editPreference($store_id, $preference = array())
	{
		...
		if (is_writable($index_path) === false) {
			throw new Exception('timezone_not_change_in_init.php');
			return false;
		} else {
			$file = $index_path;
