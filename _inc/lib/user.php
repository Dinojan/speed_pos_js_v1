...
class User{
    ...
    public function __construct($registry){
        ...
        if (isset($this->session->data['id'])) {
            $statement = $this->db->prepare("SELECT * FROM `users`  WHERE `id` = ? AND `status` = ?");
        ...
        if ($user) {
            ...
            $permissions = unserialize($roles['permission']);
	...
	{
		$statement = $this->db->prepare("SELECT * FROM `users`  WHERE (`email` = ? OR `mobile` = ?) AND `password` = ?");
		...
		if ($the_user) {
			...
			$the_roles = $statement->fetch(PDO::FETCH_ASSOC);

			$permissions = unserialize($the_roles['permission']);
	...
	{
		$statement = $this->db->prepare("SELECT `g_name` FROM `user_group` WHERE `group_id` = ?");
		$statement->execute(array((int)$this->getGroupId()));
		
		return $statement->fetch(PDO::FETCH_ASSOC)['g_name'];
	}
